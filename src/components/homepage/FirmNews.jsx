...
const FirmNews = () => {
  return (
    <div id="media" className="relative">
      ...
      <div className="container xl:max-w-[1140px] px-3 xl:px-0 mx-auto md:pt-[180px] pt-24 md:pb-[154px] sm:pb-24 pb-14 relative z-10">
        ...
        <div className="relative sm:h-[120px] h-[75px] w-full overflow-hidden">
          <div className="flex justify-center items-center flex-nowrap gap-6 news_animation absolute top-0 left-0">
            {newsMedia.map((value, index) => {
              return (
                <div
                  key={index}
                  className="md:w-[208px] w-[160px] sm:h-[120px] flex justify-center items-center group cursor-pointer"
                >
                  <Image
                    className={`group-hover:-translate-y-3 duration-300 w-[100px] h-[60px] object-contain  md:w-${value.width} md:h-${value.height}`}
                    width={value.widht}
                    height={value.height}
                    src={value.image}
                    alt="time"
